    ...
    return (
      ...
      >
        <div
          ...
          border border-blue-500 dark:border-green-400 
...
export const MovingTitle = ({ children, scrollProgress, className = "" }: MovingTitleProps) => {
  ...
  if (isFixed) {
    ...
    return (
      ...
      >
        <h2 className="text-lg font-semibold text-blue-500 dark:text-green-400 transition-colors duration-700 whitespace-nowrap">
          {children}
        </h2>
        <span className="text-sm text-blue-400 dark:text-green-300 transition-colors duration-700">
          (Front-End Developer)
        </span>
      </motion.div>
    );
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className="fixed top-0 left-0 z-50 bg-white/90 dark:bg-black/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 transition-colors duration-700"
          style={{ top: "60px" }} // Adjust based on your navbar height
        >
          <div className="flex items-center px-6 py-3">
            <div
              style={{ backgroundImage: `url(${src})` }}
              className="w-10 h-10 bg-cover bg-center rounded-full border border-blue-500 dark:border-green-400 mr-3"
            />
            <div className="flex flex-col">
              <h2 className="text-lg font-semibold text-blue-500 dark:text-green-400 transition-colors duration-700">
                {name}
              </h2>
              <span className="text-sm text-blue-400 dark:text-green-300 transition-colors duration-700">
                ({title})
